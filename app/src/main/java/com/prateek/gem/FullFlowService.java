...
public class FullFlowService extends IntentService {	
			...
			if(jsonString != null){
				try {
					...
					if(!obj.getBoolean("error")){
						...
						App.getInstance().getCurr_Members().add(insertedMember);
						db.close();						
						
						String secretCode = MEMBERADDED+DELIMITER_BW+memberIdServerAdded;
						
						db.open();
						String message = App.getInstance().getAdmin().getUserName() + " added member(s) " + App.getInstance().getCurr_group().getGroupName()+DELIMITER+secretCode;
						System.out.println(App.getInstance().getCurr_group());
						db.close();
						List<NameValuePair> notificationList = new ArrayList<NameValuePair>();
						String ids = getMembersGCMIds(App.getInstance().getCurr_group().getGroupIdServer());
		...
		if(response != null){
			try {
				...
				if(!jsonObject.getBoolean("error")){
					...
					db.close();
					db.open();
					App.getInstance().getItems().add(db.getItemByServerId(itemIdServerReceived));
					System.out.println("items"+db.getItems(App.getInstance().getCurr_group().getGroupIdServer()));
					db.close();
					
					String secretCode = ITEMADDED+DELIMITER_BW+itemIdServerReceived;
					
					db.open();
					String message = App.getInstance().getAdmin().getUserName() + " added " + itemName + " in " + App.getInstance().getCurr_group().getGroupName()+DELIMITER+secretCode;
					System.out.println(App.getInstance().getCurr_group());
					db.close();
					List<NameValuePair> notificationList = new ArrayList<NameValuePair>();
					String ids = getMembersGCMIds(App.getInstance().getCurr_group().getGroupIdServer());
		...
		if(response != null){
			try {
				...
				if(!jsonObject.getBoolean("error")){
					...
					String message = App.getInstance().getAdmin().getUserName() + " deleted " +itemName + " in " + App.getInstance().getCurr_group().getGroupName()+DELIMITER+secretCode;
					System.out.println(App.getInstance().getCurr_group());
					db.close();
					List<NameValuePair> notificationList = new ArrayList<NameValuePair>();
					String ids = getMembersGCMIds(App.getInstance().getCurr_group().getGroupIdServer());
		...
		if(response != null){
			try {
				...
				if(!jsonObject.getBoolean("error")){
					...
					String message = App.getInstance().getAdmin().getUserName() + " deleted an expense in " + App.getInstance().getCurr_group().getGroupName()+DELIMITER+secretCode;
					System.out.println(App.getInstance().getCurr_group());
		...
		if(response != null){
			try {
				...
				if(!jsonObject.getBoolean("error")){
					...
					db.open();
					int rowsUpdated = db.removeMember(memberId, App.getInstance().getCurr_group().getGroupIdServer());
					System.out.println("Rows updated "+rowsUpdated);
					db.close();
					
					String secretCode = MEMBERDELETED+DELIMITER_BW+memberServerId;
					
					String message = App.getInstance().getAdmin().getUserName() + " deleted member in " + App.getInstance().getCurr_group().getGroupName()+DELIMITER+secretCode;
					System.out.println(App.getInstance().getCurr_group());
		...
		if(response != null){
			try {
				...
				if(!jsonObject.getBoolean("error")){
					...
					db.open();
					System.out.println("expenses "+db.getExpenses(App.getInstance().getCurr_group().getGroupId()));
					db.close();
					
					String secretCode = EXPENSEADDED+DELIMITER_BW+expenseIdServerReceived;
					
					db.open();
					String message = App.getInstance().getAdmin().getUserName() + " saved an expense in " + App.getInstance().getCurr_group().getGroupName()+DELIMITER+secretCode;
					System.out.println(App.getInstance().getCurr_group());
					db.close();
					List<NameValuePair> notificationList = new ArrayList<NameValuePair>();
					String ids = getMembersGCMIds(App.getInstance().getCurr_group().getGroupIdServer());
		...
		if(response != null){
			try {
				...
				if(!jsonObject.getBoolean("error")){
					...
					db.close();
					db.open();
					System.out.println("settlements "+db.getSettlements(App.getInstance().getCurr_group().getGroupId()));
					db.close();
					
					String secretCode = SETTLEMENTADDED+DELIMITER_BW+settlementIdServerReceived;
					
					db.open();
					String message = App.getInstance().getAdmin().getUserName() + " made a settlement in " + App.getInstance().getCurr_group().getGroupName()+DELIMITER+secretCode;
					System.out.println(App.getInstance().getCurr_group());
					db.close();
	...
	private String getMembersGCMIds(Integer groupIdServer) {
		...
		for (Member member : App.getInstance().getCurr_Members()) {
			...
			if(!phoneNumber.equals(App.getInstance().getAdmin().getPhoneNumber())){
				List<NameValuePair> list = new ArrayList<NameValuePair>();
